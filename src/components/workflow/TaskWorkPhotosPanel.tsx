...
export const TaskWorkPhotosPanel: React.FC<Props> = ({ taskId, canEdit, required }) => {
  const inputRef = useRef<HTMLInputElement>(null)
  const task = useProjectWorkflowStore((s) => s.tasks[taskId])
  const addPhoto = useProjectWorkflowStore((s) => s.addWorkPhoto)
  const removePhoto = useProjectWorkflowStore((s) => s.removeWorkPhoto)
    ...
    <div className="bg-white rounded-2xl p-4 border border-gray-100 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm-mobile font-semibold text-gray-900">Фото выполненной работы</p>
        </div>
          ...
          {photos.map((url) => (
            <div key={url} className="relative aspect-square rounded-xl overflow-hidden bg-gray-100">
              ...
              {canEdit && (
                <button
                  type="button"
                  onClick={() => removePhoto(taskId, url)}
          ...
          <input
            ...
            onChange={(e) => {
              ...
              for (const f of Array.from(files)) {
                addPhoto(taskId, URL.createObjectURL(f))
              }
              e.target.value = ''
            }}
          />
      ...
      {canEdit && (
        <p className="text-xs-mobile text-gray-500">
          Фото нужны для отправки на проверку прорабу
        </p>
      )}
    </div>
  )
