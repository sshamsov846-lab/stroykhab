...
export const FineBonusPanel: React.FC<Props> = ({ workerId }) => {
  const fullName = useUserStore((s) => s.fullName)
  const addFine = useWorkerPayrollStore((s) => s.addFine)
  const addBonus = useWorkerPayrollStore((s) => s.addBonus)
  ...
  const submit = () => {
    ...
    const issuedBy = fullName || 'Прораб'
    if (mode === 'fine') {
      addFine(workerId, { amount: sum, reason: fineReason, issuedBy, comment: comment.trim() || undefined })
      toast.success('Штраф выписан')
    } else if (mode === 'bonus') {
      addBonus(workerId, { amount: sum, reason: bonusReason, issuedBy, comment: comment.trim() || undefined })
      toast.success('Премия начислена')
    }
    setAmount('')
    setComment('')
    setMode(null)
  ...
  return (
    <div className="bg-white rounded-2xl p-4 border border-gray-100 space-y-3">
      <p className="text-sm-mobile font-semibold text-gray-900">
        {mode === 'fine' ? 'Выписать штраф' : 'Выдать премию'}
      </p>
      {mode === 'fine' ? (
        <select
          value={fineReason}
