...
const STATUS_FILTERS: { id: CalculatorStatus | 'all'; label: string }[] = [
  ...
  { id: 'returned', label: 'Возвращённые' },
]

export const ForemanCalculatorReports: React.FC = () => {
  const navigate = useNavigate()
  const foremanUserKey = getCurrentUserKey()
  const getReportsForForeman = useWorkCalculatorStore((s) => s.getReportsForForeman)
  ...
  const items = useMemo(() => {
    let list = getReportsForForeman(foremanUserKey)
    ...
    return list
  }, [getReportsForForeman, foremanUserKey, statusFilter, query])

  const selected = selectedId
    ? items.find((c) => c.id === selectedId) ?? useWorkCalculatorStore.getState().getCalculator(selectedId)
    : null

  const pendingCount = getReportsForForeman(foremanUserKey).filter((c) => c.status === 'submitted').length
    ...
    <div className="pb-24 min-h-screen bg-gray-50">
      <div className="bg-primary-600 text-white px-4 pt-6 pb-4 rounded-b-3xl">
        ...
        <h1 className="text-xl-mobile font-bold flex items-center gap-2">
          ...
          Отчёты от мастеров
          {pendingCount > 0 && (
            <span className="bg-white text-primary-700 text-xs-mobile px-2 py-0.5 rounded-full">{pendingCount}</span>
          )}
        </h1>
        <p className="text-sm-mobile text-primary-100 mt-1">Проверка объёмов и формирование актов</p>
      </div>
          ...
          <>
            <div className="relative">
              ...
              <input
                ...
                placeholder="Поиск: мастер, объект…"
        ...
        {selected ? (
          <div className="bg-white rounded-2xl p-4 border border-gray-100 space-y-3">
            <CalculatorReportDetail
              ...
              footer={<ForemanCalculatorActions calc={selected} />}
            />
            <button
              type="button"
              onClick={() => navigate(`/workflow/${selected.taskId}`)}
              className="w-full text-primary-600 text-sm-mobile font-medium py-2"
            >
              Открыть задачу
            </button>
          </div>
        ) : items.length === 0 ? (
          <p className="text-center text-gray-500 text-sm-mobile py-12">Входящих отчётов нет</p>
        ) : (
          items.map((calc) => (
            <button
              key={calc.id}
            ...
            >
              <div className="flex justify-between items-start gap-2">
                <div className="min-w-0">
                  <p className="font-semibold text-gray-900">{calc.workerName}</p>
                  <p className="text-xs-mobile text-gray-500 truncate">{calc.objectName} · {calc.zoneLabel}</p>
                  <p className="text-xs-mobile text-gray-400 mt-1">
                    {new Date(calc.submittedAt ?? calc.updatedAt).toLocaleDateString('ru-RU')} · {CALCULATOR_STATUS_LABELS[calc.status]}
                  </p>
                </div>
